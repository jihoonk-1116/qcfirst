...
Next API is /login API. It supports a user who wants log in the website with his email and password. The request for the API begins with user's role, email and password. Then, The API checks if the user is registered using findOne() method. If the user was registered before signing in, a JSON Web Token(JWT) is being issued for the user with the secret key what we made. Then, the token data is being returned, and the front-end parses the response using .json() method. If the token is not error, the token is being stored it in the localStorage.
<br>
...
Then the server can check and use the token to validate the user and get user data from database. At this part, JWT decode() method is used with server secret key initialized before. If the token is valid(the user is a registered user), then the user information is being returned as JSON data. If the response is successfully transferred to the front-end, then it manipulates HTML contents using selecting class and id.   <br>
...
## Search Deliverable <a name="d6"></a>
